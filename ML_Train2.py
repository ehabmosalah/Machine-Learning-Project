...
class CorrelationFilter:
    ...
    def fit(self, data):
        ...
        numerical_data = data.select_dtypes(include=[np.number])
        # Compute correlation matrix (ignores NaNs)
        corr_matrix = numerical_data.corr()

        # Identify correlated columns to drop
    ...
    def transform(self, data):
        # Drop columns identified in fit (if they exist)
...
with open("correlation_filte_acquiring.pkl", "wb") as f:
...
class CategoryReducer:
    ...
    def fit(self, data):
        ...
        self.top_categories = (
            data[self.category_columns].sum().sort_values(ascending=False).head(self.top_n).index.tolist()
        )
...
class AgeTransformer:
    ...
    def fit(self, data):
        ...
        if not mode_series.empty:
            ...
            self.age_mode = 5  # Default fallback if all values are NaN
...
class IPOAgeTransformer:
    ...
    def fit(self, data):
        # No fitting needed, but included for pipeline compatibility
        return self

    def transform(self, data):
        df = data.copy()
        
        # Convert IPO to numeric
        df['IPO'] = pd.to_numeric(df['IPO'], errors='coerce')
        
        # Compute age
        df['age IPO'] = self.current_year - df['IPO']
        
        # Replace missing age with -1
        df['age IPO'].replace(np.nan, -1, inplace=True)
        
        # Drop original IPO column if present
...
ipo_transformer = IPOAgeTransformer(current_year=2025)
...
class EmployeeDataCleaner:
    ...
    def fit(self, data):
        ...
        self.employee_mode = (
            data['Number of Employees (year of last update)'].mode()[0]
            if not data['Number of Employees (year of last update)'].mode().empty
            else 0
        )
        # Compute mean excluding zeros for 'Number of Employees'
        self.mean_without_zeros = data.loc[
    ...
    def transform(self, data):
        ...
        # Fill missing values with mode in 'Number of Employees (year of last update)'
        ...
        # Handle nulls or values < 0 in 'Number of Employees'
        if 'Number of Employees' in df.columns:
            ...
            # Replace 0s with the precomputed mean
...
mode_values = {col: df[col].mode()[0] if not df[col].mode().empty else None 
               for col in df.columns}
...
# Select features (threshold = 0.15)
selected_features = price_correlation[price_correlation.abs() >= threshold].index.tolist()
